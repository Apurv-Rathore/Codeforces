...
@bootstrap
def dfs1(node,parent,curanswer):
    # print(node)
    answer[node] = curanswer
    for kid in adj[node]:
        if kid!=parent:
            pos = answer[node] - max(dp[kid],0)
            # print(dp[node],dp[kid])
            # dp[node]=pos
            # dp[kid]=max(pos,0)+dp[kid]
            yield dfs1(kid,node,max(pos,0)+dp[kid])
    yield 
dfs1(0,-1,dp[0])
print(*answer)
# print(*dp)
