...
def solve():
    ...
    coord = []
    for i in range(m):
        # rowsavaiable.append(l[i][1])
        coord.append([l[i][1],l[i][2]])
    # print(b)
    # print(coord)
    for i in range(n):
        for j in range(m):
            if [i,j] in coord:
                if j not in rowsavaiable:
                    rowsavaiable.append(j)
                    # print("1",i,j)
                else:
                    for k in range(m):
                        if (k not in rowsavaiable) and ([i,k] not in coord) and k!=j:
                            b[i][j],b[i][k]=b[i][k],b[i][j]
                            rowsavaiable.append(k)
                            # print("2",i,j,k)
                            break
    # print(b)
    # print(rowsavaiable)
