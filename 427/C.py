...
def dfs(node,parent):
    for kid in adj[node]:
        if kid!=parent and vis[kid]==False:
            vis[kid] = True 
            yield dfs(kid,node)
    st.append(node)
    yield 
...
@bootstrap
def dfs2(node,parent,cur,d):
    cur[0]=min(cur[0],checkpost[node])
    d[checkpost[node]]+=1
    for kid in Tadj[node]:
        if kid!=parent and vis[kid]==False:
            vis[kid] = True 
            yield  dfs2(kid,node,cur,d)
    yield 
