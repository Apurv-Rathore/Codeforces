...
def solve():
    ...
    for i in range(1,n):
        ...
        # print(*s)
        e = b[i]
        #find if it is present
        pos = s.bisect_left(e)
        # print("e,s",e,s)
        # print(valid(pos,len(s)),s,pos)
        # print(i)
        if valid(pos,len(s)) and s[pos]==e:
            #the element is present
            if pos==len(s)//2:
                #if it is already in the middle
                s.add(float('-inf'))
                s.add(float('+inf'))
                continue 
            if pos<len(s)//2:
                #if it is present at the left side of the sorted array
                d = len(s)//2-pos
                if d>2:
                    print("NO")
                    return 
            ...
            if pos<len(s)//2:
                ...
                d = pos - (len(s)//2+1)//2
                s.add(float('inf'))
            #now the element is present and now we have to add just one element
        if s[len(s)//2]!=b[i]:
            print("NO");return
        # print(*s,i)
