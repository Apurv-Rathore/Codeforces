...
void dfs(int i , int j)
{
	...
	{	
		if (l[i][j]=='#' or l[i][j]=='B' or l[i][j]=='1') {l[i][j]='1';return;}
		l[i][j]='1';
		if (i>0 and l[i-1][j]!='1') dfs(i-1,j);
    	if (j>0 and l[i][j-1]!='1') dfs(i,j-1);
    	if (i<n-1 and l[i+1][j]!='1') dfs(i+1,j);
    	if (j<m-1 and l[i][j+1]!='1') dfs(i,j+1);
	}
}
